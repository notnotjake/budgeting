...
<div class={createClass('relative flex w-xl')}>
	<div class={createClass('transition-all duration-300', testState ? 'w-4' : 'w-0')}></div>
	<div
		in:slide={{ axis: 'y', delay: 300, duration: 400 }}
		out:slide={{ axis: 'y', duration: 300 }}
		class={createClass(
			'max-h-152 min-h-50 w-full overflow-y-scroll rounded-b-4xl bg-neutral-950 text-neutral-100 shadow-lg transition-all duration-300',
			testState ? 'mt-5 rounded-t-4xl' : 'mt-0 rounded-t-none'
		)}
	>
		<div class="min-h-50 w-full p-5 pt-3">
			...
			<button onclick={testClick} class="text-rose-500">TEST</button>
