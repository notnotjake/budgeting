...
export class Sequence {
	 ...
	 * ```
	 */
	add(delay: number, action: () => void | Promise<void>, name?: string): Sequence {
	 ...
	 * ```
	 */
	at(time: number, action: () => void | Promise<void>, name?: string): Sequence {
		this.actions.push({ time, action, name })
	...
	run(): void {
		...
		// Sort actions by time to handle mixed add/at usage
		const sortedActions = [...this.actions].sort((a, b) => a.time - b.time)

		// Schedule all actions at once
