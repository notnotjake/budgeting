...
<div class="relative z-10 flex w-full items-center gap-5 px-5 py-2">
	...
	<div class="flex flex-col">
		...
		<p class="text-[1.1rem] leading-4 font-medium tracking-[-0.018em]">Today, August 30</p>
	</div>
	<div class="flex flex-col">
		<p class="font-semibold tracking-[-0.018em] text-neutral-500">Builder</p>
		<p class="text-[1.1rem] leading-4 font-medium tracking-[-0.018em]">RCI</p>
	</div>
		...
		{#each sections as section (section.id)}
			<div class="mb-3 break-inside-avoid rounded-2xl bg-neutral-200/70">
				...
				{#each section.items as item (item.id)}
					<div
						class="flex w-full items-stretch border-y-[1px] border-neutral-200 focus-within:border-blue-vibrant focus-within:bg-white"
					>
						<input
							type="text"
							class="w-15 bg-white/30 px-1.5 py-1.5 text-right font-mono text-[0.9rem] font-medium outline-none focus:bg-blue-500/20 focus:text-blue-vibrant"
						/>
